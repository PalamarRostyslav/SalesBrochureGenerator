...
from ..configs.prompts import get_brochure_generation_prompt
...
class BaseAIModel(ABC):
    ...
    def extract_links(self, website_url: str, links: List[str], use_few_shot: bool = True) -> Dict[str, Any]:
        ...
        user_prompt = f"""Here is the list of links on the website of {website_url} - please decide which of these are relevant web links for a brochure about the company, respond with the full https URL in JSON format. 
        ...
        try:
            response = self._create_completion_with_retry(messages=messages, json_mode=True)
    ...
    def generate_brochure(self, company_name: str, content: str, language: str = 'en', 
        ...
        try:
            response = self._create_completion_with_retry(messages=messages)
            result = self._extract_text_from_response(response)
